...
test("serializes separate speech requests in invocation order", async () => {
  ...
  const provider: TextToSpeechProvider = {
    id: "ordered",
    defaultSpeaker: "voice",
    maxChars: 450,
    synthesize: async (text): Promise<Audio> => {
      started.push(text);
      if (text === "First answer.") {
        markFirstStarted?.();
        await firstGate;
      }
      return { mime: "audio/wav", bytes: Buffer.alloc(64, 1) };
    },
  };
  const engine = new SpeechEngine("product", {}, provider, (audio) => emitted.push(audio.text), { betweenSentencesMs: 280, afterQuestionMs: 900 });
  const first = engine.say("First answer.", { utteranceId: "u1", turnId: "t1", purpose: "answer" });
  const second = engine.say("Second answer.", { utteranceId: "u1", turnId: "t1", purpose: "answer" });
  await firstStarted;
  assert.deepEqual(started, ["First answer."]);
  releaseFirst?.();
  await Promise.all([first, second]);
  assert.deepEqual(started, ["First answer.", "Second answer."]);
  assert.deepEqual(emitted, ["First answer.", "Second answer."]);
});
