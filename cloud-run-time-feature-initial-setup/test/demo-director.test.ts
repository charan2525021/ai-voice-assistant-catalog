...
test("verified results wait for confirmation before advancing the signed playlist", () => {
  const director = new DeterministicDemoDirector(catalog(), "member");
  let state = director.start().state;
  ...
  assert.equal(transition.state.phase, "awaiting_resume");
  assert.equal(transition.state.resumeReason, "module_complete");
  assert.deepEqual(transition.instruction, { kind: "none" });
  assert.equal(director.view(transition.state).canContinue, true);
  transition = director.control(transition.state, "continue");
...
test("failure remains conversational and can safely replace the failed module", () => {
  const director = new DeterministicDemoDirector(catalog(), "member");
  let state = director.start().state;
  state = director.captureIntake(state, "hospital").state;
  state = director.captureIntake(state, "reports").state;
  state = director.journeyResult(state, false).state;

  state = director.beginInterruptionAnswer(state).state;
  assert.equal(state.phase, "answering");
  assert.equal(state.resumeReason, "failure");
...
test("an approved replacement changes the active playlist slot", () => {
  const director = new DeterministicDemoDirector(catalog(), "member");
  let state = director.start().state;
  state = director.captureIntake(state, "hospital").state;
  state = director.captureIntake(state, "reports").state;
  ...
  assert.equal(replacement.state.phase, "playing");
  assert.equal(replacement.state.activeModuleId, "lab");
  assert.deepEqual(replacement.state.playlistModuleIds, ["lab"]);
  assert.deepEqual(replacement.instruction, { kind: "run", moduleId: "lab" });
...
test("a replacement removes the same module from a later playlist slot", () => {
  const director = new DeterministicDemoDirector(catalog(), "member");
  let state = director.start().state;
  state = director.captureIntake(state, "hospital").state;
  state = director.captureIntake(state, "show everything").state;
  state = { ...state, playlistModuleIds: ["lab", "reports"], moduleIndex: 0, activeModuleId: "lab", phase: "playing" };
  state = director.beginInterruption(state).state;
  state = director.checkpointInterruption(state, { journeyId: "show-lab", completedStepIds: [], nextStepId: "open" }).state;
  const replacement = director.replaceAfterInterruption(state, "reports");
