...
test("a finalized STT stream is released so the same SDK session can record again", async () => {
  ...
  const providers: Providers = {
    ...
    stt: { open: async (options) => {
      ...
      return { push(value) { pushed.push(Buffer.from(value)); }, finish() {}, cancel() { cancellations++; } };
    } },
    tts: { id: "sarvam", defaultSpeaker: "test", maxChars: 450, synthesize: async () => ({ bytes: Buffer.alloc(64), mime: "audio/wav" }) },
  };
  ...
  socket.send(Buffer.from("first-spoken-audio"));
  await ready;
  assert.equal(opened.length, 1);
  assert.equal(pushed[0]?.toString(), "first-spoken-audio");
  const candidate = nextSocketMessage(socket, "speech.candidate");
  opened[0].onSpeechStart();
  assert.equal((await candidate).source, "sidecar_vad");
...
test("the first completed model sentence reaches TTS before the model finishes", async () => {
  ...
  let finalTranscript: ((text: string) => void) | undefined;
  ...
  const providers: Providers = {
      ...
      step: async (_system, _messages, tools, options) => {
        if (tools.some((tool) => tool.name === "submit_turn_plan")) {
          return { texts: [], toolCalls: [{ id: "plan", name: "submit_turn_plan", args: {
            intent: "product_question", responseStrategy: "respond_answer", journeyDisposition: "journey_none",
            target: { kind: "none", id: "" }, journeyInputsJson: "{}", clarification: "",
          } }], done: false };
        }
        modelCalls += 1;
    ...
    stt: { open: async (options) => {
      finalTranscript = (text) => options.onFinal(text, {});
  ...
  await voiceReady;
  const pendingSpeech = nextSocketMessage(voice, "speech.pending");
  voice.send(JSON.stringify({ type: "voice.barge_in" }));
  await pendingSpeech;
  assert.ok(finalTranscript);
  const confirmedSpeech = nextSocketMessage(voice, "speech.confirmed");
  const cancelledForSpeech = nextSocketMessage(voice, "tts.cancel");
  finalTranscript("What is the refund policy?");
  assert.equal((await confirmedSpeech).interrupted, false);
  const cancelledGeneration = (await cancelledForSpeech).generation;
  assert.equal(typeof cancelledGeneration, "number");
  const resumedForTurn = nextSocketMessage(voice, "tts.resume");
  ...
  const resumed = await resumedForTurn;
  const audio = await firstAudio;
  assert.equal(resumed.turnId, "turn-stream");
  assert.equal(resumed.generation, cancelledGeneration);
  assert.equal(audio.generation, cancelledGeneration);
      ...
      step: async (_system, messages, tools, options) => {
        ...
        if (tools.some((tool) => tool.name === "submit_turn_plan")) {
          const args = latestText.includes("What is this product") ? {
            intent: "product_question", responseStrategy: "respond_answer", journeyDisposition: "journey_side_question",
            target: { kind: "none", id: "" }, journeyInputsJson: "{}", clarification: "",
          } : latestText.includes("Open settings") ? {
            intent: "action", responseStrategy: "respond_execute", journeyDisposition: "journey_replace",
            target: { kind: "journey", id: "show-settings" }, journeyInputsJson: "{}", clarification: "",
          } : latestText.includes("don't want") ? {
            intent: "conversation", responseStrategy: "respond_answer", journeyDisposition: "journey_stop",
            target: { kind: "none", id: "" }, journeyInputsJson: "{}", clarification: "",
          } : {
            intent: "product_question", responseStrategy: "respond_execute_then_observe_and_answer", journeyDisposition: "journey_none",
            target: { kind: "journey", id: "show-reports" }, journeyInputsJson: "{}", clarification: "",
          };
