...
function context(): DemoInterruptionAnswerContext {
  ...
  return {
    ...
    plan: { intent: "product_question", responseMode: "answer", taskControl: "side_question", playbackDirective: "resume_after_answer", needsFreshObservation: false, needsKnowledge: true, actionRequested: false, presentationRequested: false, journeyInputs: {}, policyAdjustments: [] },
...
test("the deterministic runtime—not the answer model—owns transition wording", () => {
  ...
  const paused = { ...value.plan, playbackDirective: "remain_paused" } as typeof value.plan;
  assert.equal(
    demoPlaybackTransitionText(paused, value.catalog, { journeyFailed: true, activeModuleId: "reports" }),
    "The failed module remains paused; you can retry it, skip it, or choose another section.",
  );
  assert.equal(
    demoPlaybackTransitionText(paused, value.catalog, { activeModuleId: "reports" }),
    "Would you like me to continue Smart Reports from where we paused, or explain it further?",
  );
  assert.equal(
    demoPlaybackTransitionText(paused, value.catalog, { moduleCompletedDuringInterruption: true, activeModuleId: "reports", nextModuleId: "risk" }),
    "Would you like me to continue to Diabetes Risk Calculator, or explain Smart Reports further?",
  );
  assert.equal(
    demoPlaybackTransitionText({ ...paused, answerSubjectModuleId: "risk" }, value.catalog, { activeModuleId: "reports" }),
    "Would you like me to resume Smart Reports from where we paused, or stay with Diabetes Risk Calculator?",
  );
});
