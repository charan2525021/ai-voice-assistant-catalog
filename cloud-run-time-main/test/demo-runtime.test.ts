...
test("guided demo calls the bounded planner only after verified interruption checkpoints", async () => {
  const credential = "server-only-installation-secret";
  const installation = {
  ...
  const memory = new MemoryStores([installation], [demoCatalog(true)], [], []);
  ...
  const providers: Providers = {
    model: {
      ...
      step: async (system, _messages, tools) => {
        if (tools.some((tool) => tool.name === "submit_demo_interruption_plan")) {
          demoPlannerCalls += 1;
          return { texts: [], toolCalls: [{ id: `demo-plan-${demoPlannerCalls}`, name: "submit_demo_interruption_plan", args: {
            intent: demoPlannerCalls === 1 ? "screen_question" : demoPlannerCalls === 3 ? "action" : "product_question",
            responseMode: demoPlannerCalls === 1 ? "observe_then_answer" : "answer",
            playbackDirective: demoPlannerCalls === 3 ? "remain_paused" : "resume_after_answer",
  ...
  assert.match(String((await screenAnswer).text), /continue from exactly where we paused/i);
  assert.equal((await screenResumeState).phase, "playing");
  ...
  // A second interruption is a product question. It retrieves the approved
  // signed play, answers, and automatically resumes the same checkpoint.
  statePromise = nextMessage(socket, "sable.sdk.server.demo_state");
  const secondPausePromise = nextMessage(socket, "sable.sdk.server.pause_journey");
  ...
  const productPhases = demoPhaseSequence(socket, ["awaiting_resume", "answering", "playing"]);
  ...
  assert.equal((await productPhases).at(-1)?.phase, "playing");
  ...
  const terminalAnswerPhases = demoPhaseSequence(socket, ["awaiting_resume", "answering", "playing"]);
  ...
  assert.equal((await terminalAnswerPhases).at(-1)?.phase, "playing");
  assert.match(String((await terminalAnswer).text), /easier for patients to understand/i);
  assert.match(String((await terminalAnswer).text), /next relevant part/i);
