...
export type DemoResumeReason = "interruption" | "failure";
...
export class DeterministicDemoDirector {
  ...
  beginInterruptionAnswer(value: GuidedDemoSessionState): DemoTransition {
    const state = this.restore(value);
    if (state.phase !== "awaiting_resume" || state.resumeReason !== "interruption"
      || (!state.checkpoint && !state.moduleCompletedDuringInterruption)) {
      throw new Error("An interruption answer requires a verified journey position");
    }
  ...
  finishInterruptionAnswer(value: GuidedDemoSessionState): DemoTransition {
    const state = this.restore(value);
    if (state.phase !== "answering" || state.resumeReason !== "interruption"
      || (!state.checkpoint && !state.moduleCompletedDuringInterruption)) {
  ...
  /** Replace only the active slot; the remaining signed playlist is preserved. */
  replaceAfterInterruption(value: GuidedDemoSessionState, moduleId: string): DemoTransition {
    const state = this.restore(value);
    if (state.phase !== "awaiting_resume" || state.resumeReason !== "interruption"
      || (!state.checkpoint && !state.moduleCompletedDuringInterruption) || !state.activeModuleId) {
      throw new Error("A module replacement requires a verified interrupted demo checkpoint");
    }
  ...
  journeyResult(value: GuidedDemoSessionState, ok: boolean): DemoTransition {
    ...
    return this.advanceModule(state);
  }
  ...
  view(value: GuidedDemoSessionState): Omit<DemoStateCommand, "schemaVersion" | "commandId" | "sessionId" | "sentAt" | "kind"> {
    ...
    return {
      ...
      canStart: terminal,
      canContinue: awaiting && state.resumeReason === "interruption" && (!!state.checkpoint || !!state.moduleCompletedDuringInterruption),
      canRetry: awaiting,
      canSkip: awaiting,
      canStop: !terminal,
    };
