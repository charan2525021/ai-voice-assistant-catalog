...
function scorePlay(play: CatalogSalesPlay, context: DemoSalesPlayRetrievalContext): number | undefined {
  ...
  const activeJourneyId = activeModule?.journeyId;
...
export function retrieveDemoSalesPlays(
  ...
  const selectedPlayIds = (context.catalog.payload.salesPlays ?? [])
    .map((play, index) => ({ play, index, score: allowed.has(play.kind) ? scorePlay(play, context) : undefined }))
